			...
			//add in thumbs folder if not exist
					...
					//check if file have illegal caracter
						...
						if((!($_GET['type']==1 && !$is_img) && !(($_GET['type']==3 && !$is_video) && ($_GET['type']==3 && !$is_audio))) && $class_ext>0){
							...
							$display_data = 'display: inline-block;';

								if($count_files > 30 && $is_img){
									$display_data = 'display:none;';
								}

							?>
							<li data-count="<?php echo $count_files;?>" style="<?php echo $display_data;?>" class="ff-item-type-<?php echo $class_ext;?> file <?php if(!$config['multiple_selection']){ ?>no-selector<?php } ?> <?php if($is_img){ echo ' figure-item'; } ?>"  data-name="<?php echo $file;?>" <?php if(($filter!='' && stripos($file,$filter)===false)) echo ' style="display:none;"';?>><?php
								$file_prevent_rename = false;
								$file_prevent_delete = false;
								if (isset($filePermissions[$file])) {
								...
								<div class="box">
									...
									<?php if($file == '.' || ( substr($file, 0, 1) == '.' && isset( $file_array[ 'extension' ] ) && $file_array[ 'extension' ] == fix_strtolower(trans( 'Type_dir' ) )) || (isset($file_array['extension']) && $file_array['extension']!=fix_strtolower(trans('Type_dir'))) || ($file == '..' && $subdir == '') || in_array($file, $config['hidden_folders']) || ($filter!='' && $n_files>$config['file_number_limit_js'] && $file!=".." && stripos($file,$filter)===false)){
									if($count_files > 30){
										?>
										<div style="display:inline-block; width: 100%; margin: 0 auto; text-align:center; margin-top: 5px; margin-bottom: 10px;">
											<button style="text-align:center; margin: 0 auto;" id="load-more" class="btn btn-primary" data-count="<?php echo $count_files;?>">Load More</button>
										</div>
										<?php } }?>
										...
										<script type="text/javascript">
											var limitValue  = 2;
											var offsetValue = 30;
											var configValue = parseFloat(offsetValue) * parseFloat(limitValue);

											document.getElementById("load-more").addEventListener("click", function(){

												var _this 		= document.getElementsByClassName("figure-item"),
												_this_load  	= document.getElementById("loading_container"),
												_this_container = document.getElementById("main-item-container"),
												configValue 	= parseFloat(offsetValue) * parseFloat(limitValue);

												_this_container.style.opacity = "0.7";
												this.disabled 	= true;
												
												if(_this.length > 0){
													for( i=0; i< _this.length; i++ )
													{
														var _this_is 	= _this[i];
														var _this_count = _this_is.getAttribute('data-count');

														if(_this_count >= 0 && _this_count <= configValue){
															_this[i].style.display 	= "inline-block";
														} else {
												...
												setTimeout(function(){
													_this_container.style.opacity = "1";
												},10);
