...
<div class="content-wrapper">
  ...
  <!-- Main content -->
  <form class="form-horizontal" method="POST" action="<?php echo base_url('adminsite/set_custom/edit/').$id;?>">
                  ...
                  <select class="form-control" name="karyawan_id">
                    ...
                    if(!empty($karyawan)){
                      foreach($karyawan as $index => $value){
                        $selected = (!empty($get) && $get[0]['karyawan_id'] == $value['karyawan_id']) ? 'selected' : '';
                        echo '<option '.$selected.' value="'.$value['karyawan_id'].'">'.$value['karyawan_nama'].'</option>';
                      }
                    }
                    ?>
                  </select>
                        ...
                        <?php
                          if(!empty($items)){
                            foreach($items as $index => $value){
                              echo '<tr>';
                                echo '<td>';
                                  echo '<input type="hidden" name="set_custom_detail_id[]" value="'.$value['set_custom_detail_id'].'">';
                                  echo '<input type="hidden" name="product_id[]" value="'.$value['product_id'].'">';
                                  echo '<input type="hidden" name="product_nama[]" value="'.$value['product_nama'].'">';
                                  echo $value['product_nama'].'/'.$value['product_kode']; 
                                echo '</td>';
                                echo '<td>';
                                  echo '<input type="hidden" name="product_sizestock_id[]" value="'.$value['product_sizestock_id'].'">';
                                  echo '<input type="hidden" name="product_size[]" value="'.$value['product_size'].'">';
                                  echo $value['product_size'];
                                echo '</td>';
                                echo '<td>';
                                  echo '<textarea wrap="hard" style="height: 150px;" name="note[]" class="form-control">'.$value['note'].'</textarea>';
                                echo '</td>';
                                echo '<td>';
                                  echo '<button class="remove-item-set-custom circle-button btn btn-danger btn-flat" type="button"><i class="fa fa-times"></i></button>';
                                echo '</td>';
                              echo '</tr>';
                            }
                          }
