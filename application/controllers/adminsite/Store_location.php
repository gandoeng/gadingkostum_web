...
class Store_location extends CI_Controller{
	...
	public function index(){
		...
		$data['ajax_sort_url'] 	= 'adminsite/store_location/order'; // Controller row order data
		$data['ajax_data_table']= 'adminsite/store_location/datatables'; //Controller ajax data
        ...
        //View
		$data['load_view'] = 'adminsite/v_store_location';
	...
	public function form(){
		...
		$this->form_validation->set_rules('category_name','Store Location','trim|required');
		$this->form_validation->set_rules('category_slug','Category Slug','trim|valid_url');
		$this->form_validation->set_rules('category_value_text','Store Code','trim|required');
		$this->form_validation->set_rules('category_value_textarea','Address','trim');
		...
		if($this->form_validation->run() == false){
			...
			$category_value_text 		= $this->input->post('category_value_text');
			...
			if(empty($category_id)){
				...
				$insert = array(
					...
					'category_value_text'		=> $category_value_text,
				...
				if(!empty($prod_id)){
							...
							foreach($get_product_category_detil as $index => $value){
								if(!empty($value['category_slug'])){
									if($value['flag'] == 'store_location'){
										$filter_store_location[$row][] = $value['category_slug'];
									}
								}
							}
						}
					...
					if(!empty($filter_store_location)){
						foreach($filter_store_location as $index => $value){
		...
		if(!empty($query)){
			...
			foreach($query as $index => $value){
				$data = array(
					...
					'category_value_text'		=> $value['category_value_text'],
	...
	public function datatables(){
		...
		if(!empty($query)){
			foreach($query->result() as $index => $value) {
				...
				$action = '';
				$action = '<a class="btn-edit-action btn-ajax-edit-action btn btn-primary btn-sm btn-flat" data-item="'.$value->category_id.'" data-url="'.base_url('adminsite/store_location/edit_display_data').'" style="margin-right: 5px;">Edit</a>';
				$action .= '<a class="btn-delete-action btn-ajax-delete-action btn btn-danger btn-sm btn-flat" data-item="'.$value->category_id.'" data-url="'.base_url('adminsite/store_location/delete').'">Delete</a>';
