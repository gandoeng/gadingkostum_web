...
class Size_category extends CI_Controller{
	...
	public function index(){
		...
		$data['title']  = 'Size Category';
		$data['size_category'] 			= $this->backend_model->get_sizestock_category();
		$data['table_data']		= 'category'; // element id table
		$data['ajax_sort_url'] 	= 'adminsite/size_category/order'; // Controller row order data
		$data['ajax_data_table']= 'adminsite/size_category/datatables'; //Controller ajax data
        ...
        //View
		$data['load_view'] = 'adminsite/v_size_category';
	...
	public function form(){
		...
		$this->form_validation->set_rules('category_sizestock[]','Category Sizestock','trim');
		...
		if($this->form_validation->run() == false){
			...
			$category_sizestock         = $this->input->post('category_sizestock');
			...
			if(empty($category_id)){
				...
				if(!empty($category_sizestock)){
					foreach($category_sizestock as $index => $value){
						$insert_category_sizestock = array(
							'category_sizestock' => $value,
							'category_id'		 => $id
							);
						$this->global_model->insert('category_sizestock',$insert_category_sizestock);	
					}
				}
				...
				if(!empty($prod_id)){
						...
						if(!empty($get_product_category_detil)){
							foreach($get_product_category_detil as $index => $value){
								if(!empty($value['category_slug'])){
									if($value['flag'] == 'size'){
										$filter_size[$row][] = $value['category_slug'];
									}
								}
							}
						}
					...
					if(!empty($filter_size)){
						foreach($filter_size as $index => $value){
				...
				$this->global_model->delete('category_sizestock',array('category_id' => $category_id));
	...
	public function edit_display_data(){
		...
		if(!empty($query)){
			...
			foreach($query as $index => $value){
				$query_category_sizestock = $this->backend_model->get_category_sizestock($value['category_id']);

				if(!empty($query_category_sizestock)){
					$category_sizestock = array();
					foreach($query_category_sizestock as $key => $row){
						$category_sizestock[] = $row['category_sizestock'];
					}
					$data = array(
					...
					'category_sizestock[]'		=> $category_sizestock,
	...
	public function datatables(){
		...
		if(!empty($query)){
			foreach($query->result() as $index => $value) {
				...
				$action = '';
				$action = '<a class="btn-edit-action btn-ajax-edit-action btn btn-primary btn-sm btn-flat" data-item="'.$value->category_id.'" data-url="'.base_url('adminsite/size_category/edit_display_data').'" style="margin-right: 5px;">Edit</a>';
				$action .= '<a class="btn-delete-action btn-ajax-delete-action btn btn-danger btn-sm btn-flat" data-item="'.$value->category_id.'" data-url="'.base_url('adminsite/size_category/delete').'">Delete</a>';
